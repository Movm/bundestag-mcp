/**
 * Logging utility for Bundestag MCP Server
 ...
 * Log an API request
 */
export function logApiRequest(endpoint, entity, resultCount, responseTimeMs, cached = false) {
  stats.totalRequests++;
  stats.apiRequests++;
  ...
  stats.requestsByEntity[entity] = (stats.requestsByEntity[entity] || 0) + 1;

  info('API', `Endpoint: ${endpoint} | Entity: ${entity} | Results: ${resultCount} | Time: ${responseTimeMs}ms | Cached: ${cached}`);
}
...
export function getStats() {
  ...
  return {
    ...
    breakdown: {
      ...
      byEntity: stats.requestsByEntity
    }
  };
}
